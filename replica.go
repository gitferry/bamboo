...
func (r *Replica) processCommittedBlocks(blocks []*blockchain.Block) {
	...
	if r.ID().Node() == config.Configuration.N() {
		...
		log.Warningf("[%v] Ave. delay is %v, total committed block number: %v", r.ID(), float64(r.totalDelayRounds)/float64(r.bc.GetHonestCommittedBlocks()), r.bc.GetHonestCommittedBlocks())
	}
}
