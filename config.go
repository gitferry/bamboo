...
func make_config(t *testing.T, n int, f int, unreliable bool) *config {
	ncpu_once.Do(func() {
		if runtime.NumCPU() < 2 {
...
func (cfg *config) pickByzIDs() []int {
	return RandomPick(cfg.n, cfg.f)
}

func (cfg *config) isByz(ID int) bool {
	return FindIntSlice(cfg.byzIDs, ID)
}
...
// 		for i := 0; i < cfg.n; i++ {
// 			if cfg.connected[i] {
// 				if term, leader := cfg.replicas[i].GetState(); leader {
